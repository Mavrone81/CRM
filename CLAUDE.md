...
- **Representative names map to the assigned number** (used only in message copy — there is no rep field): Number 1 → Vince/Sam, Number 2 → Vivian, Number 3 → Vicky.
- **Agreement flow.** `POST /api/wf/agreement/:id` sends the agreement PDF (from `server/data/documents/`) **via the lead's own sticky number** (`sockForLead`) with a per-lead `caption`. When a lead at status `agreement` replies with a **PDF**, the inbound handler downloads it to `server/data/signed/`, has Claude validate signature + required fields (`config.json → requiredFields`), records `wf.signed`, and auto-advances to `signed` **only if complete**. Photos/JPEGs are not validated; incomplete/failed validation stays at `agreement` and does **not** auto-reply.
