...
- **Agreement flow (Baileys-free).** `POST /api/wf/agreement/:id` no longer transmits the PDF via a socket — it **marks** the lead `agreement`, records the `caption`, and returns `{ caption, doc }`. The Pipeline card then shows a **"📄 Download PDF"** (`GET /api/documents/:id/download`, `:id` may be `default`) so the rep attaches the file manually, plus an **"Open WhatsApp"** deep link pre-filled with the caption. When a lead at status `agreement` replies with a **PDF** (either forwarded through Telegram or a rep manually logs/uploads it), Claude validates signature + required fields (`config.json → requiredFields`), records `wf.signed`, and auto-advances to `signed` **only if complete**. Photos/JPEGs are not validated; incomplete/failed validation stays at `agreement`.
